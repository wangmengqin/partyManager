<template>
	<div class="page-wrap">
		<div @click="pageUp(0)" class="li-page" :class="startDisabled?'disabled':''">首页</div>
		<div @click="pageUp(1)" class="li-page" :class="startDisabled?'disabled':''">上一页</div>  
		<div class="notPointer" :class="item==pageinationCurrentPage?'pagination_page_active':''"  
		     v-for="item in pageinationLength" @click="jump(item)">  
		  {{item}}  
		</div>  
		<div @click="pageDown(1)" class="li-page" :class="endDisabled?'disabled':''">下一页</div>  
		<div @click="pageDown(0)" class="li-page" :class="endDisabled?'disabled':''">尾页</div> 
	</div>
</template>
<script>
	export default {
